...
sourcedir = '/media/disk2/govind/work/dataset/caltech/unzip'
#outdir will have directories set00/V000/*jpg, set00/V001/ .. set01/V000/ etc 
outdir = '/media/disk2/govind/work/dataset/caltech/data/JPEGImages'
...
def open_save(seq_file, outdir, set, seq_name):  
    ...
    for idx, img in enumerate(strlist[1:]):
        ...
        filenamewithpath=os.path.join(outdir, set, seq_name, filename)         
        if print_names:
            if (idx % interval) == 0:
...
if __name__=="__main__":
    ...
    for set in sorted(sets):
        ...
        if not os.path.exists(setdir):
            print 'Cannot find diectory: %s. Skipping.' % (setdir)
            continue
